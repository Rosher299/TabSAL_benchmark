...
from .utils_train import get_model, make_dataset_from_DataCore
...
def tddpm_sample(
    ...
    D = make_dataset_from_DataCore(data)
    ...
    df = pd.DataFrame()
    if num_numerical_features < X_gen.shape[1]:
        df[data.category_column] = X_cat
    df[data.distance_column] = X_num
    # df[data.target_name] = y_gen
    df = data.format(df)
    return df
