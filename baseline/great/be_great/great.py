...
class GReaT:
    ...
    def __init__(
        ...
        **train_kwargs,
        ...
        self.llm = llm
        self.tokenizer = AutoTokenizer.from_pretrained(self.llm, device_map='cuda' if torch.cuda.is_available() else 'cpu')
        self.tokenizer.pad_token = self.tokenizer.eos_token
        self.model = AutoModelForCausalLM.from_pretrained(self.llm, device_map='cuda' if torch.cuda.is_available() else 'cpu')
    ...
    def predict(self, 
        ...
        def generate_prompt(df: pd.DataFrame, target_name): 
            prompt = []
            for _, row in df.drop(columns=[target_name]).iterrows():
                row_dict = row.to_dict()
                # 随机打乱顺序
                shuffle_idx = list(range(len(row_dict)))
                random.shuffle(shuffle_idx)
                row_dict = {list(row_dict.keys())[i]:list(row_dict.values())[i] for i in shuffle_idx}
                text = ""
            ...
            return prompt
        df_pred = []
        # 根据df_test, 生成df_test的预测结果
        for index in tqdm(range(0, len(df_test), batch_size)):
            batch = df_test.iloc[index:index+batch_size]
            prompt = generate_prompt(batch, target_name)
            prompt_tokens = _pad_tokens(self.tokenizer(prompt)["input_ids"])
            ...
            df_gen = _convert_text_to_tabular_data(text_data, pd.DataFrame(columns=df_test.columns))
            if target_name in self.num_cols:
                # 能转换为float的转换，否则置为Nan
                df_gen[target_name] = pd.to_numeric(df_gen[target_name], errors='coerce')

            # Check for NaN values and regenerate if found
            while df_gen[target_name].isna().any().any():
                nan_rows = df_gen[df_gen[target_name].isna()]
                for i in nan_rows.index:
                    ...
                    text_data = _convert_tokens_to_text(token, self.tokenizer)
                    df_gen_row = _convert_text_to_tabular_data(text_data, pd.DataFrame(columns=df_test.columns))
                    if target_name in self.num_cols:
                        # 能转换为float的转换，否则置为Nan
                        df_gen_row[target_name] = pd.to_numeric(df_gen_row[target_name], errors='coerce')
                    df_gen.loc[i] = df_gen_row.loc[0]
            df_pred.append(df_gen)
        df_pred = pd.concat(df_pred)
        df_pred = df_pred.reset_index(drop=True)
        # df_pred[target_name]类型转为与df_test[target_name]一致
        df_pred[target_name] = df_pred[target_name].astype(df_test[target_name].dtype)
        return df_pred    
